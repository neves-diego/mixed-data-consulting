"""Gera as bases brutas utilizadas na consultoria da Mixed.

Período: janeiro/2025 a junho/2026.
As inconsistências incluídas são intencionais e serão investigadas
na etapa de qualidade dos dados.
"""
...
VALORES = {"C001":5200,"C002":6500,"C003":4800,"C004":3900,"C005":7800,
           "C006":4300,"C007":4600,"C008":7200,"C009":4100,"C010":5000,
           "C011":3600,"C012":5900,"C013":3500,"C014":4700,"C015":4200}

INICIO = {"C001":"2024-01-01","C002":"2024-01-01","C003":"2024-02-01",
...
FIM = {"C013":"2025-06-30","C014":"2025-11-30","C015":"2026-03-31"}
...
def criar_clientes():
    ...
    df = pd.DataFrame(dados, columns=["cliente_id","cliente","segmento","cidade","data_entrada","status","responsavel"])
...
def criar_contratos(clientes):
    linhas = []
    numero = 1
    for cliente_id, servicos in SERVICOS.items():
        nome = clientes.loc[clientes.cliente_id == cliente_id, "cliente"].iloc[0]
        valor = round(VALORES[cliente_id] / len(servicos) / 50) * 50
        for servico in servicos:
            linhas.append((f"CTR{numero:03d}",cliente_id,nome,servico,INICIO[cliente_id],FIM.get(cliente_id,"2026-12-31"),valor,"Encerrado" if cliente_id in FIM else "Ativo"))
            numero += 1
    df = pd.DataFrame(linhas, columns=["contrato_id","cliente_id","cliente","servico","data_inicio","data_fim","valor_mensal","status_contrato"])
    ...
    for _, contrato in contratos.iterrows():
        ...
        for mes in MESES:
            if inicio <= mes <= fim:
                ...
                linhas.append((f"F{numero:04d}",contrato.cliente_id,mes,mes+pd.Timedelta(days=3),round(valor,2),"Recorrente",status))
                numero += 1
    df = pd.DataFrame(linhas, columns=["fatura_id","cliente_id","competencia","data_emissao","valor_faturado","tipo_receita","status_pagamento"])
...
def criar_projetos():
    ...
    df = pd.DataFrame(dados,columns=["projeto_id","cliente_id","projeto","servico","data_inicio","data_fim","valor_contratado","horas_estimadas","horas_realizadas","status"])
...
def criar_horas(clientes):
    ...
    atividades = {"Assessoria de imprensa":["Release","Follow-up","Clipping","Entrevista"],"Comunicação corporativa":["Planejamento","Newsletter","Comunicação institucional"],"Conteúdo":["Planejamento editorial","Copy","Social media"],"Design":["Peça digital","Apresentação","Material institucional"]}
    linhas = []
    numero = 1
    for mes in MESES:
        for profissional,cargo in EQUIPE:
            quantidade = 28 if "Diretor" in cargo else 36
            for _ in range(quantidade):
                cliente_id = np.random.choice(ativos)
                servico = np.random.choice(SERVICOS[cliente_id])
                ...
                linhas.append((f"H{numero:05d}",mes+pd.Timedelta(days=np.random.randint(0,27)),profissional,cargo,cliente_id,servico,np.random.choice(atividades[servico]),horas))
                numero += 1
    df = pd.DataFrame(linhas,columns=["registro_id","data","profissional","cargo","cliente_id","servico","atividade","horas"])
...
def criar_comercial():
    ...
    for mes in MESES:
        for _ in range(np.random.randint(5,10)):
            etapa = np.random.choice(["Contato","Reunião","Proposta","Negociação","Ganho","Perdido"],p=[.12,.18,.20,.12,.18,.20])
            resultado = "Ganho" if etapa == "Ganho" else "Perdido" if etapa == "Perdido" else "Em andamento"
            linhas.append((f"L{numero:04d}",mes+pd.Timedelta(days=np.random.randint(0,27)),f"Prospect {numero:03d}",np.random.choice(segmentos),np.random.choice(origens,p=[.30,.28,.18,.14,.10]),np.random.choice(["Marcelo","Fernanda"]),etapa,np.random.choice([2500,3500,4500,5500,6500,8000,10000]),mes+pd.Timedelta(days=np.random.randint(3,25)),resultado,np.random.choice(motivos) if resultado == "Perdido" else None))
            numero += 1
    df = pd.DataFrame(linhas,columns=["lead_id","data_entrada","empresa","segmento","origem","responsavel","etapa","valor_proposta","data_proposta","resultado","motivo_perda"])
...
def criar_entregas(clientes):
    ativos = clientes.loc[clientes.status == "Ativo", "cliente_id"].tolist()
    tipos = {"Assessoria de imprensa":["Release","Follow-up","Entrevista","Clipping","Matéria publicada"],"Comunicação corporativa":["Planejamento","Newsletter","Comunicação institucional"],"Conteúdo":["Post","Artigo","Copy","Newsletter"],"Design":["Peça digital","Apresentação","Material institucional"]}
    linhas = []
    numero = 1
    for mes in MESES:
        for cliente_id in ativos:
            for servico in SERVICOS[cliente_id]:
                for _ in range(np.random.randint(1,4)):
                    profissionais = [n for n,_ in EQUIPE if servico != "Design" or n == "Juliana"]
                    linhas.append((f"E{numero:05d}",cliente_id,mes+pd.Timedelta(days=np.random.randint(0,27)),servico,np.random.choice(tipos[servico]),f"Pauta/Projeto {numero:04d}",np.random.choice(profissionais),round(np.random.uniform(.5,3.5),1),np.random.choice(["Entregue","Publicado","Concluído","Em aprovação"])))
                    numero += 1
    df = pd.DataFrame(linhas,columns=["entrega_id","cliente_id","data","servico","tipo_entrega","tema","profissional","horas","resultado"])
...
def main():
    ...
    bases = {
        ...
        "faturamento.xlsx": criar_faturamento(contratos),
        "projetos_pontuais.xlsx": criar_projetos(),
        "horas_equipe.xlsx": criar_horas(clientes),
        "comercial.xlsx": criar_comercial(),
        "entregas_comunicacao.xlsx": criar_entregas(clientes),
    }
    for nome, tabela in bases.items():
        tabela.to_excel(PASTA_RAW / nome, index=False)
        print(f"{nome}: {len(tabela):,} registros")
    print(f"\nBases geradas em: {PASTA_RAW}")
